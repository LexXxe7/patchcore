...
class FaissNN(object):
    ...
    def run(
        ...
        """
        Returns distances and indices of the nearest neighbour search.

        Args:
            n_nearest_neighbours: Number of nearest neighbours.
            query_features: Features to retrieve.
            index_features: Index features to search in.
        """
        if index_features is None:
            return self.search_index.search(query_features, n_nearest_neighbours)

        # Builds a search index just for this search.
...
class NearestNeighbourScorer(object):
    def __init__(self, n_nearest_neighbours: int, nn_method=FaissNN()) -> None:
        """
        Nearest-Neighbour Anomaly Scorer class.

        Args:
            n_nearest_neighbours: Number of nearest neighbours used to determine
                                  anomalous pixels.
    ...
    def fit(self, detection_features: List[np.ndarray]) -> None:
        ...
        Args:
            detection_features: [list of np.ndarray]. [[bs x d_i] for i in n] Contains
                                a list of np.ndarrays for all training images
                                corresponding to respective features VECTORS (or maps,
                                but will be resized) produced by some backbone network
                                which should be used for image-level anomaly detection.
        """
    ...
    def predict(
        ...
        Args:
            query_features: List of np.ndarrays corresponding to the test features
                            generated by some backbone network.
        """
